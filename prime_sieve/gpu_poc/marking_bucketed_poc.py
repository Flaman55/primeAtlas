"""marking_bucketed_poc.py -- host harness for marking_bucketed_poc.cu (eighth-step GPU PoC).

Integrates sparse_bucketed_poc's real-hardware finding into the full two-tier pipeline: two
sweeps (n=50,000,000 real sparse-tier primes) found a non-monotonic optimum for bucket-sorting
sparse-tier write positions by shard before the atomic write -- 128 KB shards (1,048,576 bits,
9,537 buckets at combined_size=10^10) gave a real 2.34x speedup over the naive scattered write,
with clear cost mechanisms on both sides of the optimum (see marking_bucketed_poc.cu's header).

This harness drives the integrated pipeline: `marking_kernel` (dense, unchanged) for primes <
combined_size, and the new bucketed 4-step sparse pipeline (compute+count -> host prefix sum ->
scatter -> grouped write) for primes >= combined_size.
...
THREE MODES (same split as every prior file in this series):
  ...
  prime_sieve_engine_v4.c. Uses a SMALL sparse_shard_bits (16, i.e. 2 bytes) for these tiny test
  cases specifically, so the bucket-sort actually exercises multiple buckets and the real
  scatter/grouping logic -- at these cases' scale, the real-hardware-optimal 1,048,576-bit shard
  would degenerate to a single bucket (no-op sort), which would validate far less.

  STRESS mode -- the same moderate-scale run as every prior file, for a like-for-like read.

  FULL mode -- the real floor-25 scale, using the confirmed-optimal 1,048,576-bit (128 KB) shard.
  This is the number that answers whether bucketing's ~2.34x per-chunk win survives losing the
  cross-chunk upload/kernel overlap that the (now-superseded) unbucketed sparse_kernel had.

Usage (inside WSL2, after building via build_and_run_bucketed_marking.sh):
    python3 marking_bucketed_poc.py [--binary ./marking_bucketed_poc]
                                     [--engine-so ../prime_sieve_engine_v4.so]
                                     [--mode exact|stress|full|both]
                                     [--gen-threads N]   (default: os.cpu_count())
                                     [--shard-bits N]    (default: 1048576, FULL/STRESS only)
"""
import ctypes
import os
import struct
...
DEFAULT_BINARY = os.path.join(_SCRIPT_DIR, "marking_bucketed_poc")
DEFAULT_SHARD_BITS = 1 << 20  # 128 KB -- real-hardware-confirmed optimum at combined_size=1e10
EXACT_MODE_SHARD_BITS = 16    # small on purpose -- see module docstring


def write_input(path, l_final, distance_hi, distance_lo, combined_size, chunk_size,
                 num_gen_threads, sparse_shard_bits):
    with open(path, "wb") as f:
        f.write(struct.pack("<QQQQQQQ", l_final, distance_hi, distance_lo, combined_size,
                             chunk_size, num_gen_threads, sparse_shard_bits))
...
def read_output(path):
    with open(path, "rb") as f:
        ...
        (total_primes, n_dense_primes, n_sparse_primes, n_dense_chunks, n_sparse_chunks,
         n_gen_threads_used, n_sparse_buckets) = struct.unpack("<QQQQQQQ", f.read(56))
        timings = struct.unpack("<ddddddd", f.read(56))
    return bits, {
        ...
        "n_sparse_buckets": n_sparse_buckets,
    }, timings


def run_gpu(binary, l_final, distance_hi, distance_lo, combined_size, chunk_size,
            num_gen_threads, sparse_shard_bits, tag):
    in_path = os.path.join(_SCRIPT_DIR, f"_bucketed_input_{tag}.bin")
    out_path = os.path.join(_SCRIPT_DIR, f"_bucketed_output_{tag}.bin")
...
def exact_case(binary, lib, tag, name, l_final, distance_hi, distance_lo, combined_size,
    ...
    print(f"    [{name}] {status}  (dense={counts['n_dense_primes']:,} "
          f"sparse={counts['n_sparse_primes']:,} primes, n_sparse_buckets="
          f"{counts['n_sparse_buckets']}, combined_size={combined_size:,}, l_final={l_final:,})")
    return mismatches == 0


def run_exact_mode(binary, engine_so_path):
    print("=" * 78)
    print("EXACT mode -- dense (unchanged) + bucketed sparse pipeline vs. a single unchunked,")
...
def run_stress_mode(binary, gen_threads, shard_bits):
    print()
    print("=" * 78)
    print("STRESS mode -- moderate scale, dense + bucketed sparse pipeline")
    print("=" * 78)
    floor = 25
    distance = 10 ** floor
    ...
    print(f"[*] l_final={l_final:,}  combined_size={combined_size:,}  chunk_size={chunk_size:,}"
          f"  gen_threads={gen_threads}  shard_bits={shard_bits}")

    bits, counts, timings = run_gpu(
        binary, l_final, distance_hi, distance_lo, combined_size, chunk_size, gen_threads,
        shard_bits, "stress")
    (t_dense_gen, t_dense_gpu, t_sparse_gen, t_sparse_gpu, t_sparse_rt, t_download,
     t_total) = timings
    ...
    print(f"  primes: {counts['total_primes']:,} total "
          ...
          f"n_sparse_buckets={counts['n_sparse_buckets']:,}")
    ...
    print(f"  sparse: generate_wall={t_sparse_gen:.3f}s  gpu_wall={t_sparse_gpu:.3f}s "
          f"(of which roundtrip={t_sparse_rt:.3f}s)")
    print(f"  download: {t_download:.3f}s")
    print(f"  TOTAL:    {t_total:.3f}s")
    print(f"  marked-composite density: {density:.4%}")


def run_full_scale_mode(binary, gen_threads, shard_bits):
    """FULL mode (2026-08-28) -- the real floor-25 scale, with the bucketed sparse pipeline.
    Direct follow-up to marking_two_tier_poc.py's FULL-mode result: TOTAL=169.179s (pre-fix) /
    169.623s (post phase_mod fix, no measurable difference -- explained by sparse_overhead_poc's
    decomposition showing phase computation was only ~12% of sparse_kernel's cost, atomic write
    was 85.3%). This is the real test of whether bucketing's real 2.34x per-chunk win (measured
    in isolation by sparse_bucketed_poc) survives integration into the full streaming pipeline,
    where it trades away the cross-chunk upload/kernel overlap the unbucketed sparse_kernel had.
    """
    print()
    print("=" * 78)
    print("FULL mode -- REAL floor-25 scale, dense + bucketed sparse pipeline")
    print("=" * 78)
    import math
    floor = 25
    distance = 10 ** floor
    ...
    approx_pi_dense = combined_size / (math.log(combined_size) - 1)
    print(f"[*] floor={floor}  l_final={l_final:,}  combined_size={combined_size:,}  "
          f"chunk_size={chunk_size:,}  gen_threads={gen_threads}  shard_bits={shard_bits}")
    print(f"    ~{approx_pi:,.0f} primes expected total, of which ~{approx_pi_dense:,.0f} "
          f"(~{100 * approx_pi_dense / approx_pi:.2f}%) are dense-tier and the rest are "
          f"sparse-tier")
    print(f"    prior results for direct comparison: serial TOTAL=1316.729s, "
          f"parallel(no tier split) TOTAL=580.386s, two-tier(unbucketed sparse) TOTAL=169.623s")
    ...
    print(f"  primes: {counts['total_primes']:,} total "
          ...
          f"{counts['n_gen_threads_used']} generator thread(s), n_sparse_buckets="
          f"{counts['n_sparse_buckets']:,}")
    print(f"  dense:  generate_wall={t_dense_gen:.3f}s  gpu_wall={t_dense_gpu:.3f}s")
    print(f"  sparse: generate_wall={t_sparse_gen:.3f}s  gpu_wall={t_sparse_gpu:.3f}s "
          f"(of which roundtrip={t_sparse_rt:.3f}s, "
          f"{100 * t_sparse_rt / t_sparse_gpu if t_sparse_gpu else 0:.1f}% of sparse gpu_wall)")
    print(f"  download: {t_download:.3f}s")
    print(f"  TOTAL:    {t_total:.3f}s")
    print(f"  marked-composite density: {density:.4%}")
    print()
    print(f"  DIRECT COMPARISON -- production's real 2026-08-16 floor-25 benchmark: 113.352s "
          f"sieve + 62.666s write = 176.018s total. This run's TOTAL ({t_total:.3f}s) vs. the "
          f"unbucketed two-tier result (169.623s): {169.623 / t_total:.2f}x if faster. "
          f"vs. production (176.018s): {t_total / 176.018:.2f}x slower (or faster, if < 1.0).")
...
def main():
    ...
    if "--shard-bits" in sys.argv:
        shard_bits = int(sys.argv[sys.argv.index("--shard-bits") + 1])

    print(f"[*] os.cpu_count()={os.cpu_count()} -- using --gen-threads={gen_threads} for "
          f"STRESS/FULL modes (EXACT mode uses its own small, fixed per-case values and "
          f"shard_bits={EXACT_MODE_SHARD_BITS})")

    ok = True
    if mode in ("exact", "both"):
        ok = run_exact_mode(binary, engine_so) and ok
    if mode in ("stress", "both"):
        run_stress_mode(binary, gen_threads, shard_bits)
    if mode == "full":
        run_full_scale_mode(binary, gen_threads, shard_bits)
