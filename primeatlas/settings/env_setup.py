...
PrimeAtlas's backend scripts (prime_sieve/*, constellation/*) actually run INSIDE WSL,
launched from this native-Windows GUI via wsl.exe (see generation.py's
build_wsl_logged_command()/WslLoggedRunner). On a machine that already has WSL + Ubuntu +
the right system packages set up, that's invisible. On a FRESH machine it is not: the
WSL/VirtualMachinePlatform Windows features must be enabled (which requires a reboot),
Ubuntu must be installed, and python3-full, python3-numpy, libprimesieve12/-dev must be
apt-installed with ldconfig run afterward -- all BEFORE PrimeAtlas can launch anything
successfully (see INSTALL_WSL_PRIMEATLAS.md). This module automates that discovery+repair,
gated on user confirmation, so a new user only ever clicks a single "Install everything"
button (see env_setup_wizard.py for the UI) instead of following a manual checklist in a
text file.

The FULL install runs upfront, on first run, rather than lazily discovering+installing
pieces mid-task. This is forced by a real Windows constraint, not just a UX preference:
enabling the WSL/VirtualMachinePlatform optional features can require a REBOOT before
`wsl --install` will work at all (see RESTART_PENDING_EXIT_CODES below). A lazy "install
whatever's missing when a job needs it" design would have to interrupt an in-progress
generation run, ask for a reboot, and somehow resume -- much worse than blocking once, up
front, before any tab is even built. Truly optional, non-blocking extras (e.g.
libgmp-dev/libmpfr-dev for a future research module that might want arbitrary-precision C
libraries) are deliberately NOT part of this upfront set -- see REQUIRED_APT_PACKAGES's own
comment.

The WSL distro's default user is ROOT, not an interactively-created personal account. A
fresh `wsl --install -d Ubuntu` normally launches the distro once to run an OOBE step that
prompts (in a terminal) for a new UNIX username+password -- something this module cannot
answer non-interactively, and would otherwise be the one piece of this whole flow that
can't be automated. The fix used here: install with --no-launch (never triggers that OOBE
at all), then explicitly set every WSL invocation this module and the app's own launchers
use to `-u root`, plus set /etc/wsl.conf so that even a bare `wsl` terminal opens as root --
root already exists in any base Ubuntu rootfs, unrelated to the OOBE step, so this
sidesteps the interactive prompt entirely rather than trying to script an answer to it. Not
yet verified on a real machine with WSL fully absent (this sandbox has no Windows/WSL to
test against); see env_setup_wizard.py's own hand-off note.
...
# The two Windows optional features PrimeAtlas's WSL2 backend needs enabled.
# Matches INSTALL_WSL_PRIMEATLAS.md section 2.1 exactly.
...
# this app's own wsl.exe launches never activate one; see INSTALL_WSL_PRIMEATLAS.md
# section 6). Mirrors that doc's sections 4-5 exactly. Deliberately does NOT include the
# doc's optional 5.3 (libgmp-dev/libmpfr-dev) -- those are specific to research modules that
# may never be used in a given install, have no reboot/feature-enable dependency, and so are
# a good fit for on-demand, later installation (e.g. a future button next to whichever
# research sub-tab first needs them) rather than being forced on every install of what is
# meant to become a general-purpose tool.
...
def _build_install_ps1_text(distro, need_features=True, need_distro=True, need_packages=True):
    ...
    Logging design: every `exit N` here MUST remain a plain top-level statement, never
    inside a pipe or nested scriptblock. Piping this script's output into `Out-File`
    (`& script.ps1 *>&1 | Out-File ...`) to normalize its encoding would turn the script
    into a pipeline PRODUCER rather than a bare top-level script, and making the script's
    OWN exit code become the pipeline's exit code is not reliably guaranteed once it's a
    pipeline producer -- a script that correctly detects RESTART_REQUIRED and calls
    `exit 3010` can still surface to the caller as code 1. So instead of changing how the
    OUTER call captures output, logging is self-contained: this script computes its own log
    path from $PSCommandPath (which _run_elevated_ps1 computes as ps1_path + ".log" in
    Python -- same path, computed independently on each side) and appends to it itself via
    Add-Content -Encoding UTF8 through a small Log helper, used everywhere in place of a
    bare Write-Output. _run_elevated_ps1 uses a completely unadorned `-File ps1_path`
    invocation with no pipe or redirection wrapped around it at all, so
    ...
    Checklist design: run_install() decides, from a check_environment() report, which
    subset of the FULL linear sequence (features -> distro -> default-root-user -> apt
    packages) actually needs to run, rather than always running all of it. Unconditionally
    re-running `wsl --install -d Ubuntu --no-launch` against an ALREADY-installed distro
    (e.g. right after a reboot that this same install flow required) fails outright with
    Wsl/InstallDistro/ERROR_ALREADY_EXISTS, whose exit code is not stable enough to
    enumerate (a plain -1 has been observed, distinct from the 1/2 codes this script
    otherwise tolerates as harmless). need_features/need_distro/need_packages are plain
    Python booleans decided by run_install() from a check_environment() report BEFORE this
    text is even built, not runtime PowerShell conditionals -- a step that isn't needed is
    simply never emitted into the script at all, so a machine where everything is already
    present never launches an elevated process in the first place (see run_install()). As a
    second, defense-in-depth layer (in case a distro that check_environment() somehow
...
def _run_elevated_ps1(ps1_path, timeout=1800):
    ...
    Start-Process's own -RedirectStandardOutput/-RedirectStandardError/-RedirectStandardInput
    parameters are NOT supported together with -Verb RunAs -- elevation goes through
    ShellExecuteEx, which has no redirected-handle model for Start-Process to hook into.
    Combining them does not throw a terminating error; Start-Process just writes a
    non-terminating "parameter is incorrect" error to the error stream and returns $null,
    so $p.ExitCode reads as empty -- indistinguishable from the user declining the UAC
    prompt unless handled carefully, since both surface as an empty exit code.

    A bare `*>` redirect defers to Windows PowerShell 5.1's own default encoding for file
    redirection, which is UTF-16LE, not UTF-8 -- reading that back as UTF-8 decodes every
    2-byte-per-character run into mojibake.

    Piping the elevated script's output into Out-File to normalize the encoding
    (`& script.ps1 *>&1 | Out-File ...`) turns the script into a pipeline PRODUCER rather
    than a bare top-level script -- and `exit 3010` (or any other code) inside a script in
    that position does not reliably become the pipeline's/process's own exit code; a script
    that correctly detects RESTART_REQUIRED and calls `exit 3010` can still surface to the
    caller as code 1.

    All three constraints are addressed together by moving logging OUT of this function
    entirely: the elevated .ps1 script (_build_install_ps1_text) writes its own log file
    directly, at $PSCommandPath + ".log" (Add-Content -Encoding UTF8, computed from its own
    path so it lines up with log_path here without needing to pass anything in), via a
    small Log helper used everywhere in place of Write-Output -- see that function's own
    docstring. That leaves this function free to use the simplest possible invocation: a
    bare `-File ps1_path`, no pipe, no redirection wrapped around it at all, so `exit N`
    inside the script always becomes this function's own, unambiguous elevated exit code --
    the ordinary way a top-level .ps1's exit code is meant to work -- while -Verb RunAs
    -Wait -PassThru (still with no -RedirectStandard* parameter) remains the normal,
    ...
    duplicating what the wizard's own log widget already shows via the Log helper above."""
...
def run_install(distro=DEFAULT_WSL_DISTRO, timeout=1800, work_dir=None, report=None):
    ...
    Only the steps the report marks as missing are actually run, rather than always
    building and running the FULL linear sequence: unconditionally re-running
    `wsl --install -d Ubuntu --no-launch` against an ALREADY-installed distro (e.g. right
    after a reboot this same function just required) fails outright with
    Wsl/InstallDistro/ERROR_ALREADY_EXISTS (see _build_install_ps1_text's own docstring for
    the full story and the matching fix on the script-generation side). This function's own
    part of that fix is deciding, from the report, which of
