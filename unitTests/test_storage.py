...
def _test_bump_and_global():
    ...
    check(cache["10p5"]["files"] == {},
          ...
          "(see bump_floor_total's own docstring on why per-file granularity is skipped)")
    check(storage.get_global_total(cache) == (100, 1, 800),
          "bump also bumps the persisted global summary by the same delta")

    storage.bump_floor_total(cache, 5, delta_count=50, delta_file_count=1, delta_bytes=400)
    ...
    storage.bump_floor_total(cache, 9, delta_count=30, delta_file_count=1, delta_bytes=240)
    ...
    removed = storage.remove_floor_total(cache, 5)
    check(removed == (150, 2, 1200), f"remove_floor_total returns the removed floor's last totals (got {removed!r})")
    check("10p5" not in cache, "remove_floor_total drops the floor's own cache entry entirely")
    ...
    removed_again = storage.remove_floor_total(cache, 5)
    check(removed_again is None,
          "remove_floor_total on a floor with no cache entry at all returns None "
...
def _test_global_key_never_collides_with_a_real_floor():
    ...
    check(storage.GLOBAL_TOTAL_KEY == "_global",
          ...
          "list_floors()'s own naming convention)")
...
def _test_round_trip_bump_matches_full_rescan():
    ...
    run adds new window files to a floor, calling bump_floor_total() with that run's
    known delta must land on the EXACT SAME total a full update_floor_totals_cache()
    ...
    try:
        ...
        cache = {}
        total, file_count, new_read, total_bytes = storage.update_floor_totals_cache(
        ...
        # bump_floor_total() is given that delta directly instead of triggering a rescan.
        ...
        storage.bump_floor_total(cache, base_exponent, delta_count=4, delta_file_count=1,
